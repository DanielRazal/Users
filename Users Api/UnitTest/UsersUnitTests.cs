...
namespace UnitTest
{
    public class UsersUnitTests
    {
        ...
        public async Task GetAllUsers_Returns_OkResult_With_Users()
        {
            ...
            userRepositoryMock.Setup(repo => repo.GetAllUsers()).ReturnsAsync(expectedUsers);
            ...
            var json = (string)okResult.Value!;
            var returnedUsers = JsonSerializer.Deserialize<List<User>>(json!);
        ...
        public async Task GetUserById_ExistingId_Returns_OkResult_With_User()
        {
            ...
            var userRepositoryMock = new Mock<IUserRepository>();
            userRepositoryMock.Setup(repo => repo.GetUserById(userId)).ReturnsAsync(expectedUser);
        ...
        public async Task AddUser_WithExistingUsernameAndEmail_ReturnsUnauthorized()
        {
            ...
            userRepositoryMock.Verify(repo => repo.AddUser(It.IsAny<User>()), Times.Never);
            uploadsMock.Verify(uploads => uploads.UploadFile(It.IsAny<User>(), It.IsAny<IFormFile>()), Times.Never);
            emailSenderMock.Verify(sender => sender.SendEmail(It.IsAny<User>(), It.IsAny<string>(), It.IsAny<string>()), Times.Never);
        }
